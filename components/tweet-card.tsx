...
import { BadgeCheck, Heart, MessageCircle, Repeat2, Bookmark, Share, Equal } from "lucide-react";
...
export function TweetCard({ data }: TweetCardProps) {
    ...
    return (
        <div className={cn("w-full max-w-[600px] p-4 rounded-xl shadow-xl transition-colors duration-200", themeClasses[theme])} id="tweet-card">
            {/* Header */}
            <div className="flex items-center gap-3 mb-1">
                ...
                <div className="flex flex-col">
                    <div className="flex items-center gap-1 font-bold leading-tight">
                        {user.name}
                        {user.isVerified && <BadgeCheck className="w-5 h-5 text-[#1d9bf0] fill-[#1d9bf0] text-white" fill="currentColor" />}
                    </div>
                    <div className={cn("text-[15px] leading-tight", secondaryText[theme])}>@{user.username}</div>
                </div>
                <div className="ml-auto">
                    <div className={cn("p-2 rounded-full hover:bg-gray-500/10 cursor-pointer", secondaryText[theme])}>
            ...
            {content.image && (
                <div className="mt-3 relative w-full rounded-2xl overflow-hidden border border-opacity-20">
                    <Image src={content.image} alt="Tweet Image" width={500} height={500} className="w-full h-auto object-cover" />
                </div>
            )}

            {/* Meta */}
            <div className={cn("mt-4 flex items-center gap-1 text-[15px]", secondaryText[theme])}>
